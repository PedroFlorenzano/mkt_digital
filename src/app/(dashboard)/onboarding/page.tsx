...
const SECTORS = ["Tecnologia", "Saúde", "Educação", "Alimentação", "Moda", "Finanças", "Imobiliário", "Automotivo", "Beleza", "Esportes", "Entretenimento", "Outro"];
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    if (!session) return;
    fetch("/api/company").then((r) => r.ok ? r.json() : null).then((data) => {
      if (!data?.id) return;
  ...
  function handleLogo(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!["image/png", "image/jpeg", "image/webp", "image/svg+xml"].includes(file.type)) {
      setError("Formato não suportado. Use PNG, JPG, WebP ou SVG."); return;
    }
  ...
  async function handleFinish() {
    ...
    if (!res.ok) {
      ...
      setError(d.error || "Erro ao salvar empresa");
      setLoading(false);
      return;
    }
    if (logoFile) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-1 text-sm">Você pode alterar essas informações a qualquer momento</p>
        </div>
            ...
            return (
              <div key={s.label} className="flex items-center">
                <div className="flex flex-col items-center">
                  ...
                  <span className={cn("text-xs mt-1 font-medium", isActive ? "text-blue-600" : isDone ? "text-gray-600" : "text-gray-400")}>
                    {s.label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
                  <div className={cn("h-0.5 w-16 mx-2 mb-4 transition-all", step > num ? "bg-blue-600" : "bg-gray-200")} />
                )}
              </div>
            );
          })}
        ...
        <Card className="shadow-xl border-0">
          <CardContent className="p-8">
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input value={objective} onChange={(e) => setObjective(e.target.value)} placeholder="Ex: Gerar leads" />
                  </div>
                </div>
              </div>
            )}
                  ...
                  {TONES.map((t) => (
                    <button
                      ...
                      className={cn(
                        ...
                        tone === t.value ? "border-blue-600 bg-blue-50" : "border-gray-200 hover:border-gray-300 hover:bg-gray-50"
                      )}
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-lg">{t.emoji}</span>
                        <span className="font-medium text-gray-900 text-sm">{t.label}</span>
                        {tone === t.value && <Check className="h-3.5 w-3.5 text-blue-600 ml-auto" />}
                      </div>
                  ...
                  <div className="flex gap-4">
                    {colors.map((color, i) => (
                      <div key={i} className="flex flex-col items-center gap-1.5">
                        ...
                        <span className="text-xs text-gray-500">{["Principal", "Secundária", "Fundo"][i]}</span>
                        <span className="text-xs font-mono text-gray-400">{color}</span>
                      </div>
                    ))}
                    <div className="flex items-center gap-2 ml-4">
                      {colors.map((c, i) => (
                        <div key={i} className="w-10 h-10 rounded-lg border border-gray-200 shadow-sm" style={{ backgroundColor: c }} />
                      ))}
                    </div>
                  </div>
                </div>
                  ...
                  {logoPreview ? (
                    <div className="flex items-center gap-4 p-4 border-2 border-gray-200 rounded-xl bg-gray-50">
                      <div className="relative h-16 w-16 shrink-0 rounded-lg border border-gray-200 bg-white overflow-hidden">
                        <Image src={logoPreview} alt="Logo" fill sizes="64px" className="object-contain p-1" unoptimized />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900 truncate">{logoFile?.name ?? "Logo atual"}</p>
                        {logoFile && <p className="text-xs text-gray-400">{(logoFile.size / 1024).toFixed(0)} KB</p>}
                      </div>
                      <Button variant="ghost" size="sm" onClick={() => { setLogoFile(null); setLogoPreview(null); if (logoRef.current) logoRef.current.value = ""; }} className="text-red-500 hover:text-red-600 hover:bg-red-50">
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
                  ) : (
                    <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-xl p-8 cursor-pointer hover:border-blue-400 hover:bg-blue-50/30 transition-all">
                      <input ref={logoRef} type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={handleLogo} className="hidden" />
            ...
            {error && (
              <div className="mt-4 p-3 bg-red-50 border border-red-100 rounded-lg text-sm text-red-600">{error}</div>
            )}
            ...
            <div className="flex justify-between mt-8 pt-6 border-t border-gray-100">
              ...
              {step < 3 ? (
                <Button variant="gradient" onClick={() => setStep(step + 1)} disabled={step === 1 && !companyName}>
                  Próximo
                </Button>
              ) : (
                <Button variant="gradient" onClick={handleFinish} disabled={loading}>
                  {loading ? <><Loader2 className="h-4 w-4 animate-spin" />Salvando...</> : <><Check className="h-4 w-4" />Salvar configurações</>}
                </Button>
              )}
            </div>
          </CardContent>
